...
func generate ():
	...
	for i in range(data.get_vertex_count()):
		...
		#if vertex.x >= 512 or vertex.y >= 512:
		#	continue
	...
	# Access the StaticBody3D that is a child of the MeshInstance3D
	var static_body : StaticBody3D = mesh.get_child(0)

	# Retrieve the CollisionShape3D from the StaticBody3D
	var collider_shape : CollisionShape3D = static_body.get_child(0)

	collider_shape.add_to_group("TerrainMesh")
...
func get_noise_y (x, z) -> float:
	...
	if x_pixel < 512 and y_pixel < 512:
