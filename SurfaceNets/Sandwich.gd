...
func get_sample_value(index: Vector3i) -> float:
    ...
    var arbitrary_cylinder2 = arbitrary_cylinder(index, CENTER, RADIUS, a, b);
    var arbitrary_cylinder1 = arbitrary_cylinder(index, CENTER + Vector3.UP*RADIUS*3.0, RADIUS*5.0, a, b);
	return opSubtraction(arbitrary_cylinder1, arbitrary_cylinder2);

func arbitrary_cylinder(index: Vector3i, center : Vector3, radius : float, a : Vector3, b : Vector3) -> float:
	...
	var y = abs(paba-baba*0.5)-baba*0.5;
	var x2 = x*x;
	var y2 = y*y*baba;
	var min_xy2 = -min(x2,y2);
	var x2_0 = x2 if x > 0.0 else 0.0;
	var y2_0 = y2 if y > 0.0 else 0.0;
	var sum_xy_0  = x2_0 + y2_0;
	var d = min_xy2 if (max(x,y)<0.0) else sum_xy_0;
