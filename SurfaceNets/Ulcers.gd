...
const ULCER_RADIUS = 1;

const Y_PHI = 0.0;


func get_sample_value(index: Vector3i) -> float:	
	var base_sphere = CENTER.distance_to(index) - RADIUS;
	var spawn_points = generate_spawn_points();
	var result = base_sphere;
	for spawn_point in spawn_points:
		var ulcer_sphere = sphere(index, spawn_point, ULCER_RADIUS);
		# result = opUnion(ulcer_sphere, result);
		result = opSubtraction(ulcer_sphere, result);
	return CENTER.distance_to(index) - RADIUS;


func sphere(index: Vector3i, center : Vector3, radius : float) -> float:
	return CENTER.distance_to(index) - RADIUS;

func opUnion(d1 : float, d2 : float) -> float: #first value carves or counts as negative bool
	return max(d1,d2);
...
func draw_points(points : Array):
	for point in points:
		point_view.add_point(point, Colors.BLUE_B);
