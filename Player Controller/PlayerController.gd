...
func _physics_process(delta):
	# Apply gravity
	if not is_on_floor() and not dev_mode_on:
		velocity.y -= gravity * delta
	
	# Jumping
	if Input.is_action_pressed("jump") and is_on_floor() and not dev_mode_on:
	...
	if dev_mode_on:
		...
		if Input.is_action_pressed("jump"):
			vertical_movement += air_acceleration;
